...
def startedConverging(coeffList, tol):
    ...
    """
    #Use <= so that a function that is identically zero on the interval (tol == 0 because the
    #sup norm is 0) is seen to converge instead of doubling the degree guess forever.
...
def hasConverged(coeff, coeff2, tol):
    ...
    #Use <= for the same reason as in startedConverging (a function that is identically 0 has tol == 0)
